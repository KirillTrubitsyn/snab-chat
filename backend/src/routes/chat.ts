...
// чтобы модель видела весь документ. gemini-3-flash держит ~1M токенов.
...
ИЗОБРАЖЕНИЯ ИЗ БАЗЫ ЗНАНИЙ:
К некоторым документам приложены изображения их страниц (например, скриншоты интерфейса CRM-системы). Они идут после текста соответствующего документа. Это страницы документов из базы знаний, а НЕ файлы, присланные пользователем — не называй их «предоставленным скриншотом».
- НЕ начинай ответ с описания изображения. Сразу отвечай на вопрос пользователя.
- Описывай изображение, ТОЛЬКО если оно напрямую помогает ответить (например, показывает нужные кнопки, меню, поля). Тогда ссылайся так: «Как показано на странице документа N...»
- Полностью игнорируй изображения, которые не относятся к вопросу: титульные листы, обложки, нерелевантные страницы — не упоминай их вообще.`
    : "";
...
Отвечай ИСЧЕРПЫВАЮЩЕ в пределах того, что есть в <documents>. Раскрывай ВСЕ относящиеся к вопросу аспекты, которые присутствуют в документах: условия, сроки и этапы, кто принимает решение, порядок проведения, ограничения и исключения. Не ограничивайся кратким перечнем пунктов — поясняй каждый пункт по сути. Если в документах есть детали (например, на каком этапе и в каком порядке проходит процедура, какие документы оформляются) — включай их, а не опускай. Объём ответа должен соответствовать объёму релевантной информации в документах: без воды и повторов, но и без недосказанности, когда в источниках есть что добавить.

ФОРМАТ ОТВЕТА:
- Используй Markdown для форматирования
- Для дословных цитат используй формат: > "цитата"
- ОБЯЗАТЕЛЬНО указывай источник каждой дословной цитаты. Сразу после цитаты, с НОВОЙ строки и через ПУСТУЮ строку (чтобы строка не попала внутрь цитаты), добавь: «— Название документа», где Название — это имя документа-источника из атрибута filename соответствующего <document> (можно без расширения файла). НЕ используй формат [doc:N] — пиши именно название. Название документа в интерфейсе автоматически станет кликабельной ссылкой на источник.
...
${uploadedDocsContext}`;
  ...
  // C02: primary = gemini-3-flash; fallback = gemini-3.1-flash-lite on 429.
  const PRIMARY_MODEL_ID = "gemini-3-flash";
