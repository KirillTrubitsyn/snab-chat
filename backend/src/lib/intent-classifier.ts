...
Типы intent:
- "entity_lookup" — спрашивает о конкретной организации/компании группы СГК (режим закупки, структура, принадлежность, по какому ФЗ работает). Также вопросы о подрядчиках/поставщиках/исполнителях. ВАЖНО: если пользователь ищет исполнителей для конкретного вида работ или услуг (например, "услуги такси", "монтаж лифтов", "строительные работы компании", "кто оказывает услуги X") — это тоже entity_lookup, а НЕ procedure
...
export const COMPANY_PATTERNS = [
  ...
  // INN lookup
  /инн\s+\d{10}/i,
  // "услуги X компании" / "работы X компании" — ищем контрагентов по виду работ
  /услуги\s+\S+\s+компани/i,
  /работы\s+\S+\s+компани/i,
  // "кто оказывает / выполняет / делает услуги/работы"
  /кто\s+(оказыва|выполня|делает|предоставля|занимается)/i,
  // "услуги X" without procedure context (e.g., "услуги такси", "услуги клининга")
  // Only override when there's NO procedure keyword nearby
  /^услуги\s+\S+$/i,
];
...
function fallbackClassify(query: string): IntentResult {
  ...
  // Intent detection
  if (/подрядчик|контрагент|поставщик|исполнител|кто (делает|выполня|оказыва|предоставля|занимается)|найти.*(компани|организаци)|подбери.*(компани|организаци|фирм)/i.test(lower)) {
    ...
    intent = "entity_lookup";
  } else if (/услуги\s+\S+\s+компани|работы\s+\S+\s+компани/i.test(lower)) {
    intent = "entity_lookup";
