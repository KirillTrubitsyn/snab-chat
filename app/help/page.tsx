...
const sections: Section[] = [
  ...
  },
  {
    ...
    title: "Поиск контрагентов и участников закупки",
    content: (
      <>
        <P>В базе знаний хранятся карточки контрагентов — подрядчиков, поставщиков и исполнителей, с которыми работает Дирекция по закупкам. Вы можете искать информацию о компаниях, сравнивать поставщиков и подбирать потенциальных участников для новой закупки.</P>

        <H>Какую информацию можно найти</H>
        <ul style={{ margin: "8px 0", padding: "0 0 0 20px" }}>
          {[
            "Общие сведения о компании: наименование, ИНН, контакты, специализация.",
            "Опыт работы: выполненные договоры, объёмы поставок, виды работ/услуг.",
            "Оценки и отзывы: качество исполнения, соблюдение сроков, наличие рекламаций.",
            "Специализация: категории товаров, работ и услуг, которые компания предоставляет.",
          ].map((t, i) => (
            <li key={i} style={{ fontSize: 14, color: "#374151", marginBottom: 6, lineHeight: 1.6 }}>{t}</li>
          ))}
        </ul>

        <H>Примеры запросов</H>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, margin: "12px 0" }}>
          {[
            "Какие поставщики занимаются теплоизоляционными материалами?",
            "Что известно о компании ООО «Пример»?",
            "Подбери подрядчиков для выполнения строительно-монтажных работ",
            "Кто из контрагентов выполнял работы по ремонту трубопроводов?",
            "Сравни поставщиков запорной арматуры",
          ].map((q, i) => (
            <div key={i} style={{ background: "#F9FAFB", borderRadius: 8, padding: "10px 14px", fontSize: 13, fontStyle: "italic", color: "#374151", borderLeft: "3px solid #BFDBFE" }}>
              «{q}»
            </div>
          ))}
        </div>

        <H>Как искать эффективно</H>
        <StepList steps={[
          "Указывайте категорию товаров, работ или услуг — это сужает поиск до нужных контрагентов.",
          "Называйте конкретную компанию (ООО, АО) или ИНН — система найдёт её карточку.",
          "Просите сравнить нескольких поставщиков — ассистент оформит ответ в виде таблицы.",
          "Уточняйте регион, объёмы или опыт — чем конкретнее запрос, тем точнее подбор.",
        ]} />

        <Note type="info">Информация о контрагентах берётся только из карточек, загруженных в базу знаний. Если нужного поставщика нет — обратитесь к ответственному специалисту для добавления карточки.</Note>
        <Note type="tip">Удобно использовать перед началом новой закупки: узнайте, кто из проверенных поставщиков работает в нужной категории, и сформируйте список потенциальных участников.</Note>
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    content: (
      <>
        <P>В верхней панели приложения есть кнопка <strong>«Обучение»</strong>. Нажмите её — откроется образовательная платформа СнабЧат с тремя короткими курсами. Они помогут быстро разобраться с закупочной деятельностью и работать эффективнее с первых дней.</P>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, margin: "14px 0" }}>
          {[
            { num: "01", title: "Закупки по 223-ФЗ", desc: "Федеральный закон, способы закупок, пороги, роли участников. 8 разделов · 25 мин." },
            { num: "02", title: "Закупки вне 223-ФЗ", desc: "Стандарт закупок ООО СГК, пороги, процедуры, особые случаи. 9 разделов · 30 мин." },
            { num: "03", title: "Планирование закупок", desc: "Циклы планирования, дедлайны, SAP-коды, нормативные сроки. 8 разделов · 20 мин." },
          ].map(({ num, title, desc }) => (
        ...
        <Note type="tip">Каждый курс занимает 20–30 минут. Рекомендуем пройти первый курс до начала работы, остальные — по мере освоения функций.</Note>
      </>
    ),
  },
  {
