...
**Автозаполнение эталона (`POST /api/admin/kg-eval/seed-gold`).** Чтобы
не размечать gold руками, используется более сильная модель
(по умолчанию `gemini-3.1-pro-preview`, конфигурируется параметром
`goldModel`) как псевдо-эталон: она прогоняет extraction на случайной
выборке чанков (по N штук из каждого домена), результат пишется в
`kg_eval_gold` со `source = <имя модели>`. Далее обычный
`POST /api/admin/kg-eval` сравнивает продовый Flash-extractor против
этого эталона. Это даёт сигнал «насколько дешёвая модель отстаёт от
эталонной» и ловит регрессии при изменении промптов. Ручная разметка
(`source = 'manual'`) всегда приоритетнее — seed-gold никогда её не
перезаписывает. `kg_eval_run.gold_model` хранит имя модели-эталона,
против которой считались метрики (`manual` / `gemini-3.1-pro-preview` /
`mixed`).

**Рабочий цикл измерения качества.**

1. Один раз: `POST /api/admin/kg-eval/seed-gold { perDomain: 10 }` —
   получить ~60 псевдо-эталонных записей (по 10 на каждый из 6 доменов).
2. Baseline: `POST /api/admin/kg-eval` — записать текущее F1 в
   `kg_eval_run` как точку отсчёта.
3. Правка (промпт, онтология, модель) → `POST /api/admin/kg-eval` →
   сравнить с baseline через `GET /api/admin/kg-eval?limit=20`.
4. Смотреть `missingSamples` / `spuriousSamples` в `metrics` JSONB для
   точечных правок промптов.
5. Периодически (раз в квартал или при обновлении эталонной модели)
   ре-сидить gold с флагом `overwrite: true`.
