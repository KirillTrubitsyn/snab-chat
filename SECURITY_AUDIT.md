...
## Итоговый объединённый аудит (2026-04-10)

> PR conflict resolution note: этот раздел собран как единый источник правды после объединения параллельных security-PR, чтобы исключить повторяющиеся блоки и конфликтующие версии рекомендаций.

Этот раздел объединяет **оба этапа повторной проверки** и фиксирует единый финальный статус перед продакшеном.

### Что было проверено
- Ручная ревизия API-роутов `app/api/**/route.ts`, middleware и auth-хелперов.
- Проверка устойчивости RAG-контекста к prompt-injection и структурной XML-инъекции.
- Поиск потенциальных секретов в рабочем дереве и git-истории (`rg` + `git log -p --all | rg`).
- Базовая проверка сетевой поверхности в окружении контейнера (с ограничениями инструментов).
- Попытка dependency-аудита через `npm audit` (ограничено внешним 403 в окружении).

### Подтверждённые риски и что уже исправлено

1. **Prompt-injection через разрыв XML-контекста (высокий) — исправлено**
   - Добавлена более жёсткая санитизация документного контента: удаление control chars + XML escaping спецсимволов.

2. **Недостаточные лимиты auth-эндпоинтов (высокий) — исправлено**
   - Добавлены отдельные rate-limit правила для `login-password`, `verify-password`, `verify-otp`, `verify-setup-otp`.

3. **Утечка внутренних сообщений ошибок (средний/высокий) — исправлено**
   - На `upload-original` и `upload-url` ответы обезличены, внутренние DB/Storage детали больше не выдаются клиенту.

4. **Избыточная поверхность загрузки (средний) — исправлено**
   - Для `upload-url` введён whitelist MIME-типов перед выдачей signed URL.

5. **Риск эксплуатации служебной миграции в проде (средний/высокий) — исправлено**
   - `/api/migrate` отключён в production (возвращает `404`).

### Оставшиеся риски перед продом
1. **In-memory rate limiter**: per-instance модель уязвима к обходу при scale/restart.
2. **CSP с `unsafe-inline`**: остаётся XSS-риск при любой будущей инъекции.
3. **Нет edge/WAF-защиты**: отсутствует централизованная фильтрация до приложения.
4. **Secret scanning и dependency scanning не зафиксированы как CI gate**.
5. **Нет регулярного DAST/SAST цикла с блокировкой релиза при критике**.

### Единый прод-план (приоритет)

#### P0 — до релиза
- Перенести rate limiting в Redis/Upstash + edge policy.
- Включить WAF (Cloudflare/Akamai), bot protection, IP/geo правила.
- Подключить gitleaks + dependency scanning как обязательные CI checks.

#### P1 — 1–2 спринта
- Перевести CSP на nonce/hash, убрать `unsafe-inline`.
- Включить централизованные security alerts (auth-fail/OTP spikes/admin actions).
- Ограничить доступ к admin-эндпоинтам через VPN/IP allowlist.

#### P2 — регулярный цикл
- Еженедельный DAST (OWASP ZAP/nuclei), ежемесячный внешний pentest-lite.
- Ротация ключей и ревизия секретов в репозитории, CI, логах и артефактах.


---

## Повторный комплексный аудит (2026-04-11)

### Фокус проверки
- Поиск хардкоженных секретов/ключей/паролей в репозитории и git-истории.
- Анализ возможности входа и вызова API вне UI (прямые запросы, открытые эндпоинты).
- Проверка оставшихся публичных/слабо защищённых маршрутов.

### Результаты
1. **Хардкоженные рабочие ключи в коде/истории не обнаружены** по сигнатурному поиску.
   Найдены только примеры/плейсхолдеры в документации и скриптовых комментариях.

2. **Найден и закрыт открытый диагностический endpoint `/api/eval-reranker`**:
   - Был доступен без admin-проверки.
   - Добавлена обязательная проверка `requireAdmin` + отдельный strict rate-limit.

3. **Риск прямых auth-вызовов вне UI снижен**:
   - В middleware расширена Origin/Referer-защита на весь префикс `/api/auth`.
   - Это не заменяет WAF/anti-bot, но уменьшает поверхность для прямых скриптовых вызовов.

### Что всё ещё важно для продакшена
- Подключить WAF (Cloudflare) и бот-защиту на edge.
- Перевести rate-limit в distributed backend (Redis/Upstash).
- Включить регулярные DAST-сканы (ZAP/Nuclei) и CI secret scanning как обязательные gate-checks.
- Ротация ключей по расписанию (например, 90 дней) с инвентаризацией всех секретов.


### Доп. выявление по инциденту "вход через инфографику"
- В middleware проверка Origin/Referer раньше принимала `localhost` для любого окружения.
  Это позволяло scripted-клиенту подставить `Origin: http://localhost` и пройти origin-check на чувствительных POST-роутах (`/api/infographic`, `/api/auth`, и т.д.).
- Исправлено: `localhost/127.0.0.1` разрешены только в non-production.
- Для расследования добавлен аудит событий генерации инфографики (`audit_log`, action=`infographic.generate`) с сохранением actor/context метаданных.
